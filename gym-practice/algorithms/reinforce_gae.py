...
for epoch in range(num_epochs):
    ...
    for i in range(num_trajectories):
        ...
        while not episode_over:
            ...
            trajectory.append([state, observation, action, reward])
    ...
    for (trajectory, _) in batch_trajectories:

        log_prob = 0.0

        trajectory = reversed(trajectory)
        advantage = 0
        future_t_advantage = 0

        for state, next_state, action, reward in trajectory:
            ...
            # We can modify above to use the Generalized Advantage form
            # A = sum ( temporal difference residuals )

            # Compute the Temporal Difference (TD) residuals
            # r + [ \gamma V(s+1) - V(s) ]

            td_residual = reward + gamma(1) * value_fn.forward(next_state) - value_fn.forward(state)
            
            # This line is doing some heavy lifiting
            # It is giving you BOTH reward-to-go as r + g(1)r_1 + g(2)r_2 + ...
            # AND gamma weighted temporal differences of the value estimates
            # AND the ema wighting that GAE introduces
            advantage = td_residual + ema_factor * gamma(1) * future_t_advantage
            future_t_advantage = advantage
